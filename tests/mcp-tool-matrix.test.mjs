...
test("MCP dispatcher covers catalog, template, QA, infra, deploy, dashboard, and format helpers", async () => {
  ...
  assert.equal(["passed", "failed"].includes(fullCycle.status), true);
  assert.equal(fullCycle.profile.profile.id, "mcp-server");
  assert.equal(Array.isArray(fullCycle.failedCommands), true);
  ...
  assert.equal(postmortem.status, "generated");
  ...
  assert.equal(redteam.proof.results.some((scenario) => scenario.id === "destructive-tool-call"), true);
